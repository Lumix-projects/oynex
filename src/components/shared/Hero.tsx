...
export default function Hero({title, subtitle,img, button=false}:HeroProps) {
  return (
    <section className="relative w-full h-screen">
      ...
      <div className="absolute inset-0 flex flex-col gap-5 items-center justify-center text-center bg-foreground/50 px-4">
        ...
        {button && (
            <Button className="bg-main text-background">Explore Our Products</Button>
        )}
      </div>
    </section>
  )
